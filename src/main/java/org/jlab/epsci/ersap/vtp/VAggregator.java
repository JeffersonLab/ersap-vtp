...
public class VAggregator extends Thread {
     ...
     * 1 output RingBuffer.
     */
    private RingBuffer<VRingRawEvent> outputRingBuffer;

    // control for the thread termination
    private AtomicBoolean running = new AtomicBoolean(true);


    public VAggregator(RingBuffer<VRingRawEvent> ringBuffer1, RingBuffer<VRingRawEvent> ringBuffer2,
                       Sequence sequence1, Sequence sequence2,
                       SequenceBarrier barrier1, SequenceBarrier barrier2,
                       RingBuffer<VRingRawEvent> outputRingBuffer
    ) {
...
//        ringBuffer2->addGatingSequences(sequence2);
    ...
    public void get() throws InterruptedException {
        ...
        try {
            ...
            VRingRawEvent inputItem1 = ringBuffer1.get(nextSequence1);

            if (availableSequence2 < nextSequence2) {
                availableSequence2 = barrier2.waitFor(nextSequence2);
            }
            VRingRawEvent inputItem2 = ringBuffer2.get(nextSequence2);
...
*/
            ...
            if (m1.containsKey(b1) && m2.containsKey(b1)) {
                outSequence = outputRingBuffer.next();
                VRingRawEvent outputItem = outputRingBuffer.get(outSequence);
